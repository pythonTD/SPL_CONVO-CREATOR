...
public class ButtonD : Button, IDragHandler, IBeginDragHandler, IEndDragHandler // Skin, Drag & Drop
{
  ...
  public bool cursor_hand_hover = true;
  ...
  public void SetSkinByName(string name)
  {
    ...
    for (int i = 0; i < skins.Count; i++)
      if (skins[i].name == name) index = i;
    SetSkinByIndex(index);
  }
  public void SetSkinByIndex(int value)
  {
    if (value >= skins.Count) value = 0;
    index_selected = value;
    DoStateTransition(currentSelectionState, true);
  }
  public Skin GetSkinByName(string name)
  {
    return skins.Find(m => m.name == name);
  }
  protected override void DoStateTransition(SelectionState state, bool instant)
  {
    ...
    var skin = skins[index_selected >= skins.Count ? 0 : index_selected];

    // custom color
    {
      foreach (var element in skin.graphics_color)
      {
    ...
    // custom color
    {
      foreach (var element in skin.graphics_sprite)
      {
    ...
    // custom y
    {
      foreach (var element in skin.graphics_y)
      {
